...
void CudaSDFMesh::Update(float time, float4* d_outVertices, float4* d_outColors, unsigned int* d_outIndices,
    ...
    if (numActiveBlocks > 0) {
        ...
        if (numActiveBlocks > maxSafeBlocks) {
             // Only warn once per second or similar to avoid spam? For now just warn.
             // std::cerr << "Warning: Too many active blocks (" << numActiveBlocks << "), capping." << std::endl;
        ...
        // 3. Scan
        int totalItems = numActiveBlocks * SDF_BLOCK_SIZE_CUBED;
