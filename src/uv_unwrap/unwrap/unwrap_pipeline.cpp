...
UnwrapResult UnwrapPipeline::Run(const Mesh& mesh, const UnwrapConfig& cfg) {
    ...
    HarmonicFlattener flattener;
    std::vector<ChartUV> chartUVs = flattener.Flatten(mesh, charts.triChart, charts.chartCount, boundaries, cfg.useCotanWeights);
    std::cout << "Flattened charts." << std::endl;
