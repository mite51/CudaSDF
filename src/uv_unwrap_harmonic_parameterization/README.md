...
- LSCM parameterization for minimal distortion
...
### 4. **LSCM Flattening** (Updated Dec 21, 2025)
Parameterizes each chart to 2D UV space using Least Squares Conformal Maps.
- **Input:** Chart geometry + boundary loop
- **Output:** Per-chart UV coordinates (0-1 range)
- **Method:** LSCM with two-pin constraints
  - Pin two farthest vertices to remove rigid transformation ambiguity
  - Minimize conformal energy: `E = Σ Area(t) × ||∂u/∂x - ∂v/∂y||² + ||∂u/∂y + ∂v/∂x||²`
  - Solve sparse linear system: `A^T * A * x = b`
  - **Solver:** Eigen `SimplicialLDLT` (sparse Cholesky-like)

**Why LSCM?** 
- More robust than harmonic for irregular Marching Cubes triangulation
- Free boundaries (no forced circular mapping)
- Better numerical stability
- Industry standard (Blender, Maya)
- Expected: <0.1% zero-area triangles (was 13% with harmonic)
...
└── unwrap/               # Core unwrapping algorithms
    ...
    ├── flatten_lscm.h/.cpp     # LSCM parameterization
    ├── atlas_packer.h/.cpp     # Texture atlas packing
    ├── seam_splitter.h/.cpp    # Vertex splitting for seams
    └── uv_validation.h/.cpp    # Quality metrics and validation
```
...
// 3. Run pipeline (now with LSCM and automatic validation)
...
| LSCM Solve | ~500ms | O(m³) | m = avg chart verts (~1000) |
| Atlas Packing | ~5ms | O(c log c) | c = num charts (~500) |
| Vertex Split | ~10ms | O(n) | Linear scan |
| **Total** | **~700ms** | - | - |

**Bottleneck:** LSCM solver (70% of total time)
...
2. **Degenerate Geometry**: Zero-area triangles or non-manifold edges may cause issues
   - Charts with <3 vertices skip LSCM solve (UVs set to 0)
   - Automatic validation reports problematic charts
